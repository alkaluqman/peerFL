...
def createNodes(numNodes):
    nsNodes = ns.network.NodeContainer()
    nsNodes.Create(numNodes)
    return nsNodes


def createInterface(Nodes, mobility=True, loss=False, PowerStart=1, PowerEnd=1, Freq=5.180e9):
...
def act_as_server(nodes, Interfaces, server_identity, client_identity):
...
def act_as_client(nodes, client_identity):
...
def send_packet(socket, packet_content):
    print("Sending packet content")
    socket.Send(packet_content)


def send_data(socket, obj, attime=0.0):
    """pickle and send data via ns3 socket"""
    packetData = make_packets(obj)
    packetSize = 1024
    DataRate = ns.network.DataRate("512kb/s")
    for pkt in packetData:
        ns.core.Simulator.Schedule(ns.core.Seconds(attime), send_packet, socket, pkt)
        attime += packetSize * 8.0 / float(DataRate.GetBitRate())


def getRecvData(RecvData):
...
def receive_packets(socket):
    print("Receiving packet content")
    packetSize = 1024
    allPackets = []
    numRecvPacket = 0
    global rcvd_data
    ...
    if numRecvPacket == 1:  # HARDCODED
        rcvd_data = getRecvData(allPackets)
...
def recv_data(socket):
    ...
    socket.SetRecvCallback(receive_packets)
    # print(rcvd_data)
    return rcvd_data
...
def monitoring_end(flowmon_helper, flowmonitor):
