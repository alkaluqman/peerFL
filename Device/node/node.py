...
class Node:
    ...
    def training_step(self, step):
        ...
        print(
            "%sReceived object %s version %s"
            % (self.log_prefix, str(self.local_model), str(step))
        )
        self.save_model()
    ...
    def inference_step(self):
        ...
        while True:
            ...
            print(
                "%sReceived object from %s"
                % (self.log_prefix, client_identity.decode("utf-8"))
            )
            ...
            if models_received_from_clients == self.total_clients - 1:
                ...
                for i in client_list:
                    print(
                        "%sSending global model object to %s"
                        % (self.log_prefix, i.decode("utf-8"))
                    )
...
def main():
    ...
    for i in range(1, total_rounds + 1):
        ...
        if node_id == server:
            ...
            print(
                "%sTime : Inference Step = %s"
                % (this_node.log_prefix, str(time.process_time() - inference_start))
            )
