...
def make_graph(num_clients, dummy_clients, rando=1421):
    ...
    for i in range(num_clients + dummy_clients):
        graph, nodes, nodes_no = add_node(
            graph, i, random.randint(-5, 5), nodes, nodes_no
        )

    # Add the edges between the nodes by specifying
    # the from and to node along with the edge weights.

    graph, nodes, nodes_no = add_edge(
        graph,
        nodes_no,
        nodes,
        dummy_clients - 1 + 1,
    ...
    for srcNode in range(nodes_no):
        for dstNode in range(nodes_no):
            if (
                srcNode != dstNode
                and srcNode < dummy_clients
                and dstNode < dummy_clients
            ):
    ...
    for srcNode in range(nodes_no):
        ...
        for dstNode in range(nodes_no):
            ...
            if edwt == 0 or edwt >= inf:
                continue
            print(
                f"{srcNode}({nodes[srcNode]}) -> {dstNode}({nodes[dstNode]}) \t| Edge weight: {edwt}"
            )
    ...
    print(
        "\n".join(
            [
                "".join(
                    [
                        "{:4}".format(item if item < inf else float("inf"))
                        for item in row
                    ]
                )
                for row in graph
...
for i in range(num_iter):
    ...
    top_k_paths.put(
        (-1 * best_distance, best_route)
    )  # -1 factor because we want minimized values
...
for idx, (best_distance, idx_0_path) in enumerate(final_0idx_k_path):
    ...
    print(
        f"Is the distance for path number {idx+1} verified?",
        "YES! :)" if tmp == best_distance else "NO :(",
    )
