...
wifi2Interfaces = address.Assign(
    wifi2StaDevices
)  # we only care about this cuz server will be installed here
...
echoClient1 = ns.applications.UdpEchoClientHelper(
    wifi2Interfaces.GetAddress(nWifi2 - 1), 9
)
...
echoClient2 = ns.applications.UdpEchoClientHelper(
    wifi2Interfaces.GetAddress(nWifi2 - 1), 9
)
...
class SimpleMLP:
    ...
    def build(shape, classes, only_digits=True):
        base_model_1 = VGG16(
            include_top=False, input_shape=(32, 32, 3), classes=y_train.shape[1]
        )
        model_1 = Sequential()
